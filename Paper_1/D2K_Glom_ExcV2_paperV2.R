...
# Set wd (output) ----------------------------------------------------------
dir <- "/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2"
setwd(dir)
...
asvdf <- asvdf %>% dplyr::select(-(d2k.rem$colnames.asvdf.))


# If the column names (with subject IDs) line up among the tables, this command will return 'TRUE' and character(0), respectively:
qc.physeq1 <- all(colnames(asvdf) %in% mapdf$StudyID) %>% all
qc.physeq1
qc.physeq2 <- mapdf$StudyID[!(mapdf$StudyID %in% colnames(asvdf))] %>% length
...
qc.physeq3 <- all(rownames(mapdf) %in% colnames(asvdf))
qc.physeq3
qc.physeq4 <- all(colnames(asvdf) %in% rownames(mapdf))
qc.physeq4
...
if(qc.physeq1 == TRUE & qc.physeq2 == 0 & qc.physeq3 == TRUE & qc.physeq4 == TRUE){
  qc.physeq = TRUE
}

if(qc.physeq1 == FALSE | qc.physeq2 != 0 | qc.physeq3 == FALSE | qc.physeq4 == FALSE){
  qc.physeq = FALSE
}

qc.physeq
if(qc.physeq == TRUE){
  print("Physeq object generation qc passed. Generating physeq object: dat")
...
# Resolve to Genus: Remove empty taxa ----------------------------------------------------
#QC
...
# Extract Taxa frequency and Mean relative abundance - empty taxa removed ----------------------
physeq <- dat_empty_rel
...
tiff("meanRA_prop_density.tiff", height = 500, width = 1200, res = 90)
...
tiff(paste0("meanRA_prop_scatter.tiff"), height = 500, width = 500, res = 90)
p
dev.off()
...
#Pass criteria taxa
dat_g1_pass_rel <- prune_taxa(pass$OTU, dat_empty_rel)
dat_g1_pass_counts <- prune_taxa(pass$OTU, dat_empty)
...
#Fail criteria taxa
dat_g1_fail_rel <- prune_taxa(fail$OTU, dat_empty_rel)
dat_g1_fail_counts <- prune_taxa(fail$OTU, dat_empty)
...
nrow(fail) == dat_g1_fail_rel %>% tax_table %>% nrow() #should return TRUE
nrow(fail) == dat_g1_fail_counts %>% tax_table %>% nrow() #should return TRUE 
...
melt_rel_group = rbind(psmelt(dat_g1_pass_rel), psmelt(dat_g1_fail_rel)) %>% group_by(Sample) %>% #QC relative abundance; should be 1 because haven't discarded genus yet
  dplyr::summarize(total_Abundance = sum(Abundance))
melt_rel_group
summary(melt_rel_group %>% select(-Sample))
...
dat_g1_fail_rel <- tax_glom(dat_g1_fail_rel, NArm = FALSE, taxrank = "Genus")
dat_g1_fail_counts <- tax_glom(dat_g1_fail_counts, NArm = FALSE, taxrank = "Genus")


#List of taxa postglom
taxrel <- dat_g1_fail_rel %>% tax_table %>% as.data.frame() %>% mutate(taxa_full = paste(Kingdom, Phylum, Class, Order, Family, Genus, sep = "_")) %>% rownames_to_column("sp")
taxcount <- dat_g1_fail_counts %>% tax_table %>% as.data.frame() %>% mutate(taxa_full = paste(Kingdom, Phylum, Class, Order, Family, Genus, sep = "_")) %>% rownames_to_column("sp")
diff <- setdiff(taxrel$taxa_full, taxcount$taxa_full)
taxrel$sp == taxcount$sp #TRUE = perfect match

#Conversion between relative taxa sp# and count taxa sp#
tax.conversion <- left_join(taxrel %>% select(sp, taxa_full) %>% rename(Rel.OTU = sp), 
                            taxcount %>% select(sp, taxa_full) %>% rename(Count.OTU = sp), 
                            by = "taxa_full")
cbind(taxrel$sp, taxcount$sp) %>% head
...
nrow(pass)
nrow(fail)
nrow(rel.criteria) #should be the number of glommed genus
