...
# Set wd -------------------------------------------------------------------
parent.dir <- "/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna"
setwd(parent.dir)
...
metadata <- c("Illumina.Plate.Number", "Date.on.Sample", "record", "visit", "age", "dsex" , "hisp_ethnicity",
              "race", "depression", "anxiety", "anhedonia", "dars17_total", "gad7_total", "phq9_total", "mood") 
numeric_trait <-c("dars17_total", "gad7_total", "phq9_total", "age") 
...
#Replace taxa with abbreviated taxa names


# Prep relabund df --------------------------------------------------------
tax.conversion <- read.csv("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/ResolveGenus/taxaconversion_postglom.csv")
rel <- psmelt(dat_rel_glom) %>%
  left_join(tax.conversion %>% rename(OTU = Rel.OTU), by = "OTU") %>% 
  select(-OTU) %>% rename(OTU = Count.OTU) %>% 
  group_by(StudyID) %>% select(-all_of(rm_col)) %>%
  arrange(Sample, desc(Abundance))%>%
  group_by(Sample) %>% 
  filter(OTU != "other") %>%
...
network_modules <- function(net_type, minModuleSize, softPower,
  ...
  if(export == TRUE){
    ...
    plotTOM = dissTOM^-1 # Transform dissTOM with a power to make moderately strong connections more visible in the heatmap
...
module_stability <- function(data.input = datExpr_parent, 
  ...
  for (k in 1:nboot){
    ...
    #Goal: 
    #Original module colour may not be the same colour as the most similar bootstrapped module
    #Address this by sequentially assignment of bootstrap statistics to original modules
    #Start with the boostrap module we are most confident is similar to the original module (i.e. highest jaccard similarity)
    #Originally considered removing og.module and bootstrap module colours once they've been assigned.
    #However, if there are fewer modules identified in the boostrap module, this will mean that one original module will not get a jaccard similarity statistic for that round 
    #Instead, retain all boostrap modules even after they've been assigned. If there are fewer modules in the bootsrapped sample, this will be reflected as poor cluster stability in at least one of the two original clusters that got merged, and indicate to the user to choose a different module size. 
    
    #Code: 
    #All taxa
    head(jaccard.matrix %>% arrange(desc(j.stat)))
    dumdf <- jaccard.matrix %>% group_by(og.module) %>% slice(which.max(j.stat)) %>% mutate(bootstrap = k) #assigns bootstrap module to original module
  ...
  if(export == TRUE){
    ...
    write.csv(hub.summary, "ValidateHubTaxa_summary.csv")
    write.csv(bootstrap.result.members, "ValidateModuleMembers.csv")
    write.csv(bootstrap.result.hub, "ValidateHubTaxa.csv")
  }
  
  return(list(member.summary, hub.summary, bootstrap.module, bootstrap.result.hub))
}
...
#size = 10
network_modules(net_type = net, minModuleSize = 10, softPower = 3)
module_traits()
investigate_module(clin.trait = c("phq9_total", "gad7_total", "age"), mod = c("green", "brown", "yellow"))
module_stability(net = "signed", input.minModulesize = 10, input.softpower = 3) 

#size = 15
network_modules(net_type = net, minModuleSize = 15, softPower = 3)
module_traits()
investigate_module(clin.trait = c("phq9_total", "gad7_total", "age"), mod = c("blue", "yellow"))
module_stability(net = "signed", input.minModulesize = 15, input.softpower = 3) 

cyto_fun(module = "blue", netname = "signed_mindmod50")


#size = 50
network_modules(net_type = net, minModuleSize = 50, softPower = 3)
module_traits()
investigate_module(clin.trait = c("phq9_total", "gad7_total", "age"), mod = "blue")
module_stability(net = "signed", input.minModulesize = 50, input.softpower = 3) 
}


# Run unsigned network ----------------------------------------------------
select_network <- FALSE
if(select_network == TRUE){
#=== Select soft-thresholding === 
net <- "unsigned"
network_selection(net_type = net)
...
network_modules(net_type = net, minModuleSize = 10, softPower = 3)
module_traits()
investigate_module(clin.trait = c("phq9_total", "gad7_total", "age"), mod = c("blue", "brown", "yellow"))
module_stability(net = "unsigned", input.minModulesize = 10, input.softpower = 3) 
...
module_traits()
investigate_module(clin.trait = c("phq9_total", "gad7_total", "age"), mod = "blue")
module_stability(net = "unsigned", input.minModulesize = 50, input.softpower = 3) 
}
...
network_modules(net_type = "signed", minModuleSize = 50, softPower = 3)
module_traits()
investigate_module(clin.trait = c("phq9_total", "gad7_total", "age"), mod = c("brown"))
...
setwd()
#Age
cor.test(datTraits$gad7_total, datTraits$age, method = "pearson") #R^2 = -0.12 p = 0.084
cor.test(datTraits$phq9_total, datTraits$age, method = "pearson") #R^2 = -0.11 p= 0.12

#GAD/PHQ9
cor.test(datTraits$gad7_total, datTraits$phq9_total, method = "pearson") #R^2 = 0.77 p = 2.2E-16
...
hubhex <- c("#634D6A", "#E4645A", "#0D7488")

# Top taxa - brown module -----------------------------------------------------------
#Import clincally important taxa list 
age.important <- read.csv("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/signed/minmod50/brown/age/brown_age_TOPclinicaltaxa.csv") %>% data.frame %>% rename(taxa = X)
gad.important <- read.csv("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/signed/minmod50/brown/gad7_total/brown_gad7_total_TOPclinicaltaxa.csv") %>% data.frame %>% rename(taxa = X)
phq.important <- read.csv("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/signed/minmod50/brown/phq9_total/brown_phq9_total_TOPclinicaltaxa.csv") %>% data.frame %>% rename(taxa = X)

#Import brown module hub taxa
brown.hub <- read.csv("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/signed/minmod50/brown/brown_hubtaxa.csv") %>% data.frame %>% rename(taxa = X)

#CLR dataset
clr[1:10, 1:20] #df

#Set wd for exports
setwd("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/signed/minmod50/brown")
...
hub_taxa_density(df = clr, taxa = brown.hub %>% select(taxa),
                 module = "brown", export_params = c(3500, #width
                                                         1000, #height
                                                         300, #res
                                                         3), #ncol 
                 hex = hubhex, 
...
#Plot clinical taxa (correlation)
important_taxa_scatter(df = "clr", trait = "gad7_total", taxa = gad.important %>% select(taxa, GS.gad7_total), 
                       module = "brown", export_params = c(8300, #width
                                                           1000, #height
                                                           300, #res
                                                           10 #ncol 
                                                           ), 
...
important_taxa_scatter(df = "clr", trait = "phq9_total", taxa = phq.important %>% select(taxa, GS.phq9_total), 
                       module = "brown", export_params = c(8300, #width
                                                           1000, #height
                                                           300, #res
                                                           10 #ncol 
...
important_taxa_scatter(df = "clr", trait = "age", taxa = age.important %>% select(taxa, GS.age), 
                       module = "brown", export_params = c(8300, #width
                                                           1000, #height
                                                           300, #res
                                                           10 #ncol 
...
mod10.stability <- read.csv("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/signed/minmod10/ValidateModuleMembers_summary.csv") %>% stabimport
mod15.stability <- read.csv("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/signed/minmod15/ValidateModuleMembers_summary.csv") %>% stabimport
mod50.stability <- read.csv("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/signed/minmod50/ValidateModuleMembers_summary.csv") %>% stabimport
...
#Set wd for exports
setwd("/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/signed")
...
head(geneModuleMembership.global)
head(moduleTaxa.global)
head(datExpr)
head(datTraits)
head(brown.hub)

#Plot  function
scatter_taxasig_hub <- function(taxa_mm = geneModuleMembership.global, #taxa module membership dataframe
  ...
  voi <- as.data.frame(eval(parse(text = paste0 ("datTraits$", trait))))
  taxa_clinsig = as.data.frame(cor(datExpr, voi, use = "p"))
  
  #Setup plotting dataframe
  modNames = substring(names(MEs), 3) #Remove ME from module names
  column = match(module, modNames) #Locate column belonging to module
  
  df <- data.frame(abs(taxa_mm[moduleTaxa.global, column]), 
                   taxa_clinsig[moduleTaxa.global, 1]) %>% mutate_all(as.numeric) %>% mutate(taxa = taxa_mm[moduleTaxa.global,] %>% rownames,)
  colnames(df) <- c("taxa_mod_membership", "clinical_correlation", "taxa")
  ...
  p <- ggscatter(x = "taxa_mod_membership", 
    ...
    geom_point(data = df_highlight,
               ...
               size = 2.5, shape = 16) + 
    scale_color_manual(values=c(hubhex)) + 
    scale_fill_manual(values = c(hubhex))
  print(p)
}
...
tiff("brown_taxasig.tiff", width = 1200, height = 4000, res = 300)
...
# Run Age-corrected Network ------------------------------------------------------
# === Age adjusted data === 
parent.dir <- "/Users/sar/Dropbox/Dallas2K/PhyloseqV3/Paper_v2/wcna/age-corrected"

setwd(parent.dir)
#Remove sample with age = NA 
datExpr_age <- clr %>% filter(!(is.na(age))) %>% select(-metadata) %>% 
  as.matrix() %>% as.data.frame() %>% column_to_rownames(var = "Sample") %>% mutate_all(as.numeric)
datTraits_age <- clr %>% select(numeric_trait) %>% filter(!(is.na(age))) %>%
  as.matrix() %>% as.data.frame() %>% column_to_rownames(var = "Sample") %>% mutate_all(as.numeric)
(rownames(datExpr_age) == rownames(datTraits_age)) %>% all #qc data is aligned

datAdj <- empiricalBayesLM(data = datExpr_age,
                           removedCovariates = datTraits_age %>% pull(age), verbose = 10)

#Input data for WCNA
datExpr <- datAdj$adjustedData
datTraits <- datTraits_age %>% select(-age)
traitColors <- WGCNA::numbers2colors(datTraits, signed = FALSE)
...
network_modules(net_type = net, minModuleSize = 15, softPower = 3)
module_traits()
investigate_module(clin.trait = c("phq9_total", "gad7_total"), mod = c("blue", "yellow"))
module_stability(net = "signed", input.minModulesize = 15, input.softpower = 3) 

network_modules(net_type = net, minModuleSize = 25, softPower = 3)
module_traits()
investigate_module(clin.trait = c("phq9_total", "gad7_total"), mod = c("brown"))
module_stability(net = "signed", input.minModulesize = 25, input.softpower = 3) 
